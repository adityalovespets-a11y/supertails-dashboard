...
function renderDashboard(slice,g){
  ...
  // granVal: average per day across the selected slice (single day = absolute).
  // This makes signal-card numbers consistent with the chart's selected window
  // and comparable to the daily baseline regardless of granularity.
  ...
  // granBase (for top signal cards):
  //   Always return DAILY baseline since granVal() now returns daily avg.
  //   Charts use scaledBase(wk) at chart-rendering time separately.
  function granBase(wkBase){
    if(!wkBase) return 0;
    return wkBase/7;
  }
  // chartBase: scale a weekly baseline to match the chart's granularity
  // (used by chart bars where weekly view sums 7 days into one bar)
  function chartBase(wkBase){
    if(!wkBase) return 0;
  ...
  // ── BLR Orders top card ──────────────────────────────────────────────────
  {
    const BLR_ORD_BASE_DAILY = 753; // Mar 1-22 gross (matches BLR_ORD_BASE in section 8)
    const ablr = granVal(slice.orders_blr);
    const blrBase = BLR_ORD_BASE_DAILY;
  ...
  charts.forEach(c=>{
    ...
    const bg=color+'26';
    const scaledBase=chartBase(c.base);
  ...
  // Installs chart — Total + Organic as two series
  {
    ...
    const orgBase = chartBase(baselines.direct_installs_india||baselines.direct_installs_bangalore||0);
    mkChart('ch2', iL, [
